...
COIN_TGZ='https://github.com/BitcoinZeroOfficial/bitcoinzero/releases/download/5.0.1.0/linux-x64.tar.gz'
COIN_ZIP=$(echo $COIN_TGZ | awk -F'/' '{print $NF}')
COIN_NAME='BitcoinZero'
COIN_PORT=29301
RPC_PORT=29302
...
function create_key() {
  echo -e "Enter your ${RED}$COIN_NAME bznode Private Key${NC}. Leave it blank to generate a new ${RED}bznode Private Key${NC} for you:"
  read -e COINKEY
  if [[ -z "$COINKEY" ]]; then
  ...
  COINKEY=$($COIN_PATH$COIN_CLI bznode genkey)
  if [ "$?" -gt "0" ];
    then
...
echo -e "Prepare the system to install ${GREEN}$COIN_NAME${NC} bznode."
apt-get update
...
libevent-pthreads-2.0-5 libevent-core-2.0-5 libminiupnpc-dev libzmq3-dev git nano tmux curl wget pwgen libzmq3-dev libboost-all-dev libboost-system-dev libboost-filesystem-dev libboost-chrono-dev libboost-program-options-dev libboost-test-dev libboost-thread-dev
...
function important_information() {
 echo -e "================================================================================================================================"
 echo -e "$COIN_NAME bznode is up and running listening on port ${RED}$COIN_PORT${NC}."
 echo -e "Configuration file is: ${RED}$CONFIGFOLDER/$CONFIG_FILE${NC}"
 echo -e "Start: ${RED}systemctl start $COIN_NAME.service${NC}"
 echo -e "Stop: ${RED}systemctl stop $COIN_NAME.service${NC}"
 echo -e "VPS_IP:PORT ${RED}$NODEIP:$COIN_PORT${NC}"
 echo -e "BZNODE PRIVATEKEY is: ${RED}$COINKEY${NC}"
 ...
 echo -e "Use ${RED}$COIN_CLI bznode status${NC} to check your MN."
